...
export async function mountEvents({map, L}) {
    ...
    const panel = node('dialog', '', 'events-panel');
    panel.id = 'events-dialog';
    panel.setAttribute('aria-labelledby', 'events-title');
    let opener = document.getElementById('open-events');
    if (!opener) {
        opener = node('button', 'Open Events', 'live-chat-link');
        opener.id = 'open-events'; opener.type = 'button';
    ...
    const titleText = node('h1', 'Campus events'); titleText.id = 'events-title';
    title.append(node('small', 'EXPLORE FIU'), titleText);
    const close = node('button', '×', 'events-close'); close.type = 'button';
    ...
    function eventCard(e, inPopup=false) {
        ...
        if(!inPopup && isMappable(e)) {
            ...
            button.onclick = () => {
                const marker = markers.get(e.building_id || e.id);
                if (marker) {
                    panel.close();
    ...
    function render() {
        ...
        for(const b of buildings) {
            ...
            marker.bindPopup(popup(b,events),{maxHeight:330,maxWidth:330,autoPanPaddingTopLeft:[10,20],autoPanPaddingBottomRight:[20,20]});markers.set(b.abbreviation,marker);
        }
