...
try {
    ...
    for (const visibility of ['public','private']) {
        ...
        const another = await alice.service.sendMessage(alice.user, 'Closed group', id);
        await seed('chats/' + id, {name:text(id),visibility:text(visibility),lastActivityAt:{timestampValue:new Date(Date.now()-86400000).toISOString()},idleHours:{integerValue:'12'}});
        await denied(rawRemoval(mod, another.id, id));
    }
    ...
    console.log('PASS: regular/verifier/unsigned/impersonator blocked; protected roles; atomic immutable audits; forged removals denied; member-only group moderation; private DMs; bulk clear; new arrivals preserved; revocation.');
