...
function element() { return {
    value: '', textContent: '', children: [], events: {}, hidden: false, open: false,
...
(async () => {
    ...
    assert.equal(person.children[0].children[0].textContent, '<Bob>');
    ...
    assert.equal(elements['direct-list'].children.length, 1);
    ...
    assert.equal(elements['members-list'].children.find(row => row.children[0].children[0].textContent.includes('(you)')).children[0].disabled, true);
    const member = elements['members-list'].children.find(row => row.children[0].children[0].textContent === '<Bob>');
    await member.children[1].events.click(); assert.equal(removed, 1);
