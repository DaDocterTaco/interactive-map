...
export function mountForums({ user }) {
    ...
    function listenPosts() {
        ...
        stopPosts = service.watchPosts(postLimit, (data, cached) => {
            ...
            if (active && !disposed && version === listVersion) $("forum-list-status").textContent = failure(error);
        });
    }
    ...
    on("forum-post-form", "submit", async event => {
        ...
        try {
            ...
            if (!disposed) $("forum-compose-status").textContent = `Post not saved. ${error.message || failure(error)}`;
        } finally {
            posting = false;
            if (!disposed) {
    ...
    on("forum-reply-form", "submit", async event => {
        ...
        try {
            ...
            if (active && version === threadVersion) $("forum-reply-status").textContent = "Reply posted.";
        } catch (error) {
            if (!disposed && selected === id) $("forum-reply-status").textContent = `Reply not saved. ${error.message || failure(error)}`;
        } finally {
            replying = false;
            if (!disposed) {
